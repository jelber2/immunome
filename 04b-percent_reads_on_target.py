...
04b-percent_reads_on_target.py

Command:
cd InDir = /work/jelber2/immunome/stampy-alignment
1.Bedtools intersect
        ~/bin/bedtools2-2.19.1/bin/bedtools intersect -abam Sample.stampy.bam \
...
find . -name '*.bam' -not -name '*paired*' -not -name '*singles*' -not -name '*intersect*' -exec ~/scripts/immunome/04b-percent_reads_on_target.py {} \;
...
if len(sys.argv)<2:
    ...
    for InFileName in FileList: # so samtools grabs only the file names (i.e., Samples)
        FileSuffix = ".stampy.bam" # string to remove from InFileName
        FilePrefix = "./"
        ...
        Command ="""
        ~/bin/bedtools2-2.19.1/bin/bedtools intersect -abam %s.stampy.bam -b /work/jelber2/reference/immunome_baits_C_picta-3.0.3.bed > %s.intersect.bam
