...
1.Produce only sam header and count number of header lines with wc, also copy
  header file to stampy-alignment directory for bam concatenation later
        ~/bin/samtools-0.1.19/samtools view -SH \
...
4.Add the header back to each split
        ~/bin/parallel-20140622/src/parallel 'cat Sample.header.bwa.sam {} > {}.sam' ::: Sample-*

5.Convert the split sam files to bam format and remove sam in filename
        ~/bin/parallel-20140622/src/parallel '~/bin/samtools-0.1.19/samtools view -Sb \
        {} > {}.bwa.bam' ::: Sample-*.sam
        ~/bin/parallel-20140622/src/parallel 'rename .sam "" {}' ::: Sample-*.sam.bwa.bam

6.Map the bam files using stampy
        ~/bin/parallel-20140622/src/parallel 'python ~/bin/stampy-1.0.23/stampy.py \
...
8.Rename bwa.bam from Sample-##.bwa.bam.stampy.sam
        cd ../stampy-alignment/
        ~/bin/parallel-20140622/src/parallel 'rename .bwa.bam "" {}' ::: Sample-*.bwa.bam.stampy.sam

9.Convert sam2bam then remove sam from filename
        ~/bin/parallel-20140622/src/parallel '~/bin/samtools-0.1.19/samtools view \
        -bS \
        -t RefDir/C_picta-3.0.3.fai {} > {}.bam' ::: Sample-*.stampy.sam
        ~/bin/parallel-20140622/src/parallel 'rename .sam "" {}' ::: Sample-*stampy.sam.bam

10.Concatenate bam files
        ~/bin/samtools-0.1.19/samtools cat \
        -h Sample.header.sam \
        -o Sample.stampy.bam \
...
Directory info:
InDir = /work/jelber2/immunome/bwa-alignment
Input Files = Sample-singlesANDmerged.bwa.sam
              Sample-paired.bwa.sam
OutDir = /work/jelber2/immunome/stampy-alignment
...
~/scripts/immunome/03-stampy.py *.bwa.sam
...
if len(sys.argv)<2:
    ...
    for InFileName in FileList: # so stampy processes all bam files
        ...
        ~/bin/parallel-20140622/src/parallel 'cat %s.header.bwa.sam {} > {}.sam' ::: %s-*

        ~/bin/parallel-20140622/src/parallel '~/bin/samtools-0.1.19/samtools view -Sb \
        {} > {}.bwa.bam' ::: %s-*.sam
        ~/bin/parallel-20140622/src/parallel 'rename .sam "" {}' ::: %s-*.sam.bwa.bam
        ...
        cd ../stampy-alignment/
        ~/bin/parallel-20140622/src/parallel 'rename .bwa.bam "" {}' ::: %s-*.bwa.bam.stampy.sam

        ~/bin/parallel-20140622/src/parallel '~/bin/samtools-0.1.19/samtools view \
        -bS {} > {}.bam' ::: %s-*.stampy.sam
        ~/bin/parallel-20140622/src/parallel 'rename .sam "" {}' ::: %s-*stampy.sam.bam
